...
export default function StatisticsTab({ data, loading, drawName }: StatisticsTabProps) {
  ...
  useEffect(() => {
    if (data.length > 0) {
      setNumberStats(calculateNumberStats(data));
    } else {
      setNumberStats([]);
    }
  }, [data]);
    ...
    <div className="space-y-6">
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Analyse de la fréquence d'apparition des numéros (Gagnants et Machine).</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <h3 className="text-xl font-semibold mb-4 mt-8">Fréquence de tous les numéros</h3>
          <div className="h-[500px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 60 }}>
                ...
                <Bar dataKey="Gagnants" fill="var(--color-chart-1, #8884d8)" name="Tirages Gagnants" />
                <Bar dataKey="Machine" fill="var(--color-chart-2, #82ca9d)" name="Tirages Machine" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle>Tableau Détaillé des Fréquences</CardTitle>
        </CardHeader>
        <CardContent>
